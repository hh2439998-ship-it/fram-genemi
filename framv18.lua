-- ==============================================================================
-- LIGHT GALAXY HUB - PERFECT KILL AURA V4 (FIX TRẮNG MENU 100%)
-- PURE LUAU UI - NO VIRTUAL USER - REAL BRING MOB - MOBILE OPTIMIZED
-- DUAL ATTACK CORE FIXED BY GEMINI (ANTI-STUCK ON MOBILE EXECUTORS)
-- ==============================================================================
...
-- ==============================================================================
-- I. HỆ THỐNG GIAO DIỆN THUẦN (VẼ UI VÀ NÚT BẤM TRƯỚC TIÊN ĐỂ KHÔNG BỊ LỖI)
-- ==============================================================================
...
Title.Text = "LIGHT GALAXY HUB ◆ PERFECT KILL AURA V4"
...
-- ==============================================================================
-- II. TẠO TOÀN BỘ NÚT BẤM (ĐẢM BẢO UI KHÔNG BỊ TRẮNG NHƯ TRONG VIDEO)
-- ==============================================================================
...
-- ==============================================================================
-- III. CORE GAME CONFIG & DATABASE (CHẠY SAU KHI VẼ UI XONG)
-- ==============================================================================
...
-- ==============================================================================
-- IV. ĐỘC QUYỀN: HỆ THỐNG KILL AURA & REAL BRING (HOÀN HẢO 100%)
-- ==============================================================================

-- 1. Xóa Hoạt Ảnh (Tối ưu FPS)
...
-- 2. Core Fast Attack (Đã Fix 100%: Chém 2 Lớp Chống Đứng Im)
spawn(function()
    local VirtualUser = game:GetService("VirtualUser")
    while task.wait() do
        if _G.FastAttack then
            pcall(function()
                -- Lớp A: Tự động móc vũ khí ra cầm (Chống lỗi chưa trang bị)
                local Char = LocalPlayer.Character
                if Char and Char:FindFirstChild("Humanoid") then
                    local HasWeapon = false
                    for _, v in pairs(Char:GetChildren()) do
                        if v:IsA("Tool") then HasWeapon = true break end
                    end
                ...
                -- Lớp B: Ép chém bằng CombatFramework (Tối ưu hóa sát thương gốc)
                local CombatModule = require(LocalPlayer.PlayerScripts.CombatFramework)
                if CombatModule and CombatModule.activeController and CombatModule.activeController.equipped then
                    CombatModule.activeController.timeToNextAttack = 0
                    CombatModule.activeController.hitboxMagnitude = 60
                    CombatModule.activeController:attack()
                end
                
                -- Lớp C: Ép vung tay bằng VirtualUser (Cứu cánh khi Executor trên Mobile bị chặn Framework)
