-- ==============================================================================
-- LIGHT GALAXY HUB ◆ V15 ĐỈNH CAO (FULL AUTO SEA 1-3, SEA EVENT, RAID, FRUIT)
-- KHÔNG CẮT XÉN - TỰ ĐỘNG CHUYỂN SEA - AUTO TIỀN BIỂN 6 FULL SKILL
-- ==============================================================================
...
-- [TỰ ĐỘNG VÀO PHE HẢI TẶC VÀ NHẬN DIỆN SEA]
...
local Main = Instance.new("Frame", Hub) Main.Size = UDim2.new(0, 600, 0, 360) Main.Position = UDim2.new(0.15, 0, 0.2, 0) Main.BackgroundColor3 = Color3.fromRGB(15, 15, 25) Main.Active = true Main.Draggable = true Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 10)
...
local Title = Instance.new("TextLabel", Top) Title.Size = UDim2.new(1, -30, 1, 0) Title.Position = UDim2.new(0, 15, 0, 0) Title.BackgroundTransparency = 1 Title.Font = Enum.Font.GothamBold Title.Text = "LIGHT GALAXY HUB ◆ V15 ĐỈNH CAO" Title.TextColor3 = Color3.fromRGB(0, 255, 255) Title.TextSize = 15 Title.TextXAlignment = Enum.TextXAlignment.Left
local TabSc = Instance.new("ScrollingFrame", Main) TabSc.Size = UDim2.new(0, 140, 1, -50) TabSc.Position = UDim2.new(0, 5, 0, 45) TabSc.BackgroundTransparency = 1 TabSc.ScrollBarThickness = 2 Instance.new("UIListLayout", TabSc).Padding = UDim.new(0, 5)
...
local function CreatePage(Name)
    local Pg = Instance.new("ScrollingFrame", Cont) Pg.Size = UDim2.new(1, 0, 1, 0) Pg.CanvasSize = UDim2.new(0, 0, 6, 0) Pg.BackgroundTransparency = 1 Pg.ScrollBarThickness = 4 Pg.Visible = false
    ...
    local Btn = Instance.new("TextButton", TabSc) Btn.Size = UDim2.new(1, -10, 0, 35) Btn.BackgroundColor3 = Color3.fromRGB(35, 35, 60) Btn.Font = Enum.Font.GothamSemibold Btn.Text = Name Btn.TextColor3 = Color3.fromRGB(200, 200, 200) Btn.TextSize = 12 Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 6)
...
local function AddToggle(Pg, Txt, Var)
    _G[Var] = false
    local Frm = Instance.new("Frame", Pg) Frm.Size = UDim2.new(1, -16, 0, 40) Frm.BackgroundColor3 = Color3.fromRGB(30, 30, 50) Instance.new("UICorner", Frm).CornerRadius = UDim.new(0, 6)
    local Lbl = Instance.new("TextLabel", Frm) Lbl.Size = UDim2.new(0.7, 0, 1, 0) Lbl.Position = UDim2.new(0, 10, 0, 0) Lbl.BackgroundTransparency = 1 Lbl.Font = Enum.Font.Gotham Lbl.Text = Txt Lbl.TextColor3 = Color3.fromRGB(255, 255, 255) Lbl.TextSize = 13 Lbl.TextXAlignment = Enum.TextXAlignment.Left
    local Btn = Instance.new("TextButton", Frm) Btn.Size = UDim2.new(0, 50, 0, 24) Btn.Position = UDim2.new(1, -60, 0, 8) Btn.BackgroundColor3 = Color3.fromRGB(150, 50, 50) Btn.Font = Enum.Font.GothamBold Btn.Text = "OFF" Btn.TextColor3 = Color3.fromRGB(255, 255, 255) Btn.TextSize = 11 Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 4)
...
local function AddDropdown(Pg, Txt, Options, Var)
    _G[Var] = Options[1]
    local Frm = Instance.new("Frame", Pg) Frm.Size = UDim2.new(1, -16, 0, 40) Frm.BackgroundColor3 = Color3.fromRGB(30, 30, 50) Instance.new("UICorner", Frm).CornerRadius = UDim.new(0, 6)
    local Lbl = Instance.new("TextLabel", Frm) Lbl.Size = UDim2.new(0.5, 0, 1, 0) Lbl.Position = UDim2.new(0, 10, 0, 0) Lbl.BackgroundTransparency = 1 Lbl.Font = Enum.Font.Gotham Lbl.Text = Txt Lbl.TextColor3 = Color3.fromRGB(255, 255, 255) Lbl.TextSize = 13 Lbl.TextXAlignment = Enum.TextXAlignment.Left
    local Btn = Instance.new("TextButton", Frm) Btn.Size = UDim2.new(0, 100, 0, 24) Btn.Position = UDim2.new(1, -110, 0, 8) Btn.BackgroundColor3 = Color3.fromRGB(50, 40, 90) Btn.Font = Enum.Font.GothamBold Btn.Text = _G[Var] Btn.TextColor3 = Color3.fromRGB(255, 255, 255) Btn.TextSize = 11 Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 4)
    local idx = 1 Btn.MouseButton1Click:Connect(function() idx = idx + 1 if idx > #Options then idx = 1 end _G[Var] = Options[idx] Btn.Text = _G[Var] end)
end

local pFarm = CreatePage("Auto Farm Level")
local pSea = CreatePage("Săn Tiền & Sea")
local pRaid = CreatePage("Raid & Trái Cây")
local pHack = CreatePage("ESP & PVP Hacks")
Pages["Auto Farm Level"].Visible = true

_G.FastAttackRedz = true
_G.BringMobSafe = true
_G.AutoFarmLevel = false
_G.AutoRaid = false
_G.AutoFruit = false
_G.AutoSeaEvent = false
_G.AutoSeaMoney = false
_G.SelectWeaponSea = "Blox Fruit"
_G.AutoV3 = false
_G.AutoV4 = false
_G.ESPPlayer = false
_G.ESPChest = false
_G.ESPFruit = false
_G.AutoRejoin30m = true

AddToggle(pFarm, "[BẬT] Fast Attack Bypass (Đâm Netcode)", "FastAttackRedz")
AddToggle(pFarm, "[BẬT] Real Bring Mob 8m (Gom Quái)", "BringMobSafe")
AddToggle(pFarm, "Auto Farm Level (Tự Động Full Map 1-3)", "AutoFarmLevel")
AddToggle(pFarm, "Tự Động Chuyển Sea (Sea 1 -> 2 -> 3)", "AutoTravelSea")

AddToggle(pSea, "Auto Săn Sự Kiện Biển (Shark, Piranha)", "AutoSeaEvent")
AddToggle(pSea, "Auto Săn Sea Beast (Biển Cấp 6 Săn Tiền)", "AutoSeaMoney")
AddDropdown(pSea, "Vũ Khí Săn Sea Beast:", {"Blox Fruit", "Melee", "Sword", "Gun"}, "SelectWeaponSea")
AddToggle(pSea, "Auto Bật Tộc V3 (Khi đánh Boss/Sea)", "AutoV3")
AddToggle(pSea, "Auto Bật Tộc V4 (Khi Full Năng Lượng)", "AutoV4")

AddToggle(pRaid, "Auto Đi Raid (Tự Đánh & Qua Đảo)", "AutoRaid")
AddToggle(pRaid, "Auto Nhặt & Cất Trái Ác Quỷ", "AutoFruit")

AddToggle(pHack, "ESP Người Chơi (Xuyên Tường)", "ESPPlayer")
AddToggle(pHack, "ESP Rương & Trái Ác Quỷ", "ESPChest")
AddToggle(pHack, "Chống Admin & Server Hop 30P", "AutoRejoin30m")
-- LIGHT GALAXY HUB V15 - PHẦN 2: DỮ LIỆU LEVEL FARM & AUTO TRAVEL SEA
...
-- Bộ Dữ liệu Full 100% Cấp độ từ Redz Hub
...
-- TỰ ĐỘNG CHUYỂN SEA (Khi đủ level max của khu vực)
task.spawn(function()
    while task.wait(5) do
        if _G.AutoTravelSea then
            local Lvl = LocalPlayer.Data.Level.Value
            if World1 and Lvl >= 700 then
                CommF:InvokeServer("TravelDressrosa")
            elseif World2 and Lvl >= 1500 then
                CommF:InvokeServer("TravelZou")
            end
        end
    end
end)
-- LIGHT GALAXY HUB V15 - PHẦN 3: CORE HACKS, SEA EVENT CÀY TIỀN BIỂN 6
        ...
        if _G.FastAttackRedz then
            pcall(function()
                ...
                for _, mob in pairs(Workspace.Enemies:GetChildren()) do
                    ...
                    if eRoot and mob.Humanoid.Health > 0 and (eRoot.Position - root.Position).Magnitude <= 65 then
...
-- BRING MOB 8M (An toàn tuyệt đối)
task.spawn(function()
    ...
    while task.wait(0.2) do
        ...
        if _G.AutoV4 then
            pcall(function()
                ...
                if char and char:FindFirstChild("RaceEnergy") and char.RaceEnergy.Value >= 1 and not char.RaceTransformed.Value then
                    VIM:SendKeyEvent(true, Enum.KeyCode.Y, false, game) task.wait(0.1) VIM:SendKeyEvent(false, Enum.KeyCode.Y, false, game)
                end
            end)
        end
    end
...
-- AUTO NHẬN NHIỆM VỤ & FARM LEVEL THEO DATA
task.spawn(function()
    while task.wait(0.2) do
        if _G.AutoFarmLevel then
            pcall(function()
                ...
                if not LocalPlayer.PlayerGui.Main.Quest.Visible then
                    TweenTo(NPC_IP)
                    if (LocalPlayer.Character.HumanoidRootPart.Position - NPC_IP.Position).Magnitude < 15 then CommF:InvokeServer("StartQuest", QName, QNum) end
                else
                    local Target = nil
                    for _, Enemy in pairs(Workspace.Enemies:GetChildren()) do
                        if string.find(Enemy.Name, MobName) and Enemy.Humanoid.Health > 0 then Target = Enemy break end
                    end
                    if Target then TweenTo(Target.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) else TweenTo(Mob_IP * CFrame.new(0, 45, 0)) end
                end
            end)
        end
    end
...
-- AUTO FARM TIỀN BIỂN 6 (SĂN SEA BEAST TỰ ĐỘNG XẢ SKILL)
task.spawn(function()
    while task.wait(0.1) do
            ...
            pcall(function()
                ...
                if Workspace:FindFirstChild("SeaBeasts") then
                    for _, sb in pairs(Workspace.SeaBeasts:GetChildren()) do
                        if sb:FindFirstChild("HumanoidRootPart") and sb:FindFirstChild("Humanoid") and sb.Humanoid.Health > 0 then SeaBeast = sb break end
                    end
                end
                
                if SeaBeast then
                    TweenTo(SeaBeast.HumanoidRootPart.CFrame * CFrame.new(0, 60, 0)) -- Bay lơ lửng né sát thương biển
                    -- Cầm vũ khí săn biển
                    local Char = LocalPlayer.Character
                    if Char and (not Char:FindFirstChildOfClass("Tool") or not string.find(Char:FindFirstChildOfClass("Tool").Name, _G.SelectWeaponSea)) then
                        for _, T in pairs(LocalPlayer.Backpack:GetChildren()) do
                            if T:IsA("Tool") and (T.ToolTip == _G.SelectWeaponSea or string.find(T.Name, _G.SelectWeaponSea)) then Char.Humanoid:EquipTool(T) break end
                        end
                    end
                    -- Xả full skill Z, X, C, V
                    for _, key in ipairs({Enum.KeyCode.Z, Enum.KeyCode.X, Enum.KeyCode.C, Enum.KeyCode.V}) do
                        VIM:SendKeyEvent(true, key, false, game) task.wait(0.1) VIM:SendKeyEvent(false, key, false, game)
                    end
                    VirtualUser:CaptureController() VirtualUser:ClickButton1(Vector2.new(850, 850))
                else
                    -- Không có Sea Beast -> Ra thẳng Biển cấp 6 (CFrame xa map)
                    local Sea6CFrame = CFrame.new(-30000, 100, 30000)
                    if (LocalPlayer.Character.HumanoidRootPart.Position - Sea6CFrame.Position).Magnitude > 500 then TweenTo(Sea6CFrame) end
                end
            end)
        end
    end
    ...
    while task.wait(1) do
        if _G.AutoRaid then
            pcall(function()
                ...
                for _, island in pairs(Workspace:GetChildren()) do if string.find(island.Name, "Island") then RaidIsland = island break end end
                if RaidIsland and RaidIsland:FindFirstChild("Hitbox") then TweenTo(RaidIsland.Hitbox.CFrame * CFrame.new(0, 40, 0)) end
                for _, mob in pairs(Workspace.Enemies:GetChildren()) do
                    if mob:FindFirstChild("HumanoidRootPart") and mob.Humanoid.Health > 0 then _G.BringMobSafe = true TweenTo(mob.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) end
                end
            end)
        end
        if _G.AutoFruit then
            pcall(function()
                for _, f in pairs(Workspace:GetChildren()) do if f:IsA("Tool") and string.find(f.Name, "Fruit") and f:FindFirstChild("Handle") then TweenTo(f.Handle.CFrame) task.wait(0.5) CommF:InvokeServer("StoreFruit", f:GetAttribute("OriginalName") or f.Name, f) end end
            end)
        end
    end
end)

-- TỰ ĐỘNG CLICK MÀN HÌNH CHỐNG VĂNG
task.spawn(function()
    while task.wait(0.1) do
        if _G.AutoFarmLevel or _G.AutoBone or _G.AutoCake or _G.AutoRaid or _G.AutoSeaEvent then
            pcall(function()
                local Char = LocalPlayer.Character
...
task.spawn(function()
    while task.wait(1) do
        if _G.InfSoru and LocalPlayer.Character then
            pcall(function() for _, f in next, getgc() do if type(f) == "function" and getfenv(f).script == LocalPlayer.Character:WaitForChild("Soru") then for i, v in pairs(debug.getupvalues(f)) do if type(v) == "table" and v.LastUse then debug.setupvalue(f, i, {LastAfter = 0, LastUse = 0}) end end end end end)
        end
        if _G.NoDodgeCD and LocalPlayer.Character then
            pcall(function() for _, f in next, getgc() do if type(f) == "function" and getfenv(f).script == LocalPlayer.Character:WaitForChild("Dodge") then for i, v in next, debug.getupvalues(f) do if tostring(v) == "0.4" then debug.setupvalue(f, i, 0) end end end end end)
        end
        if _G.ESPChest then
            for _, c in pairs(game:GetService("CollectionService"):GetTagged("_ChestTagged")) do
                if not c:FindFirstChild("ESPC") then local bg = Instance.new("BillboardGui", c) bg.Name = "ESPC" bg.Size = UDim2.new(0, 100, 0, 40) bg.AlwaysOnTop = true local txt = Instance.new("TextLabel", bg) txt.Size = UDim2.new(1,0,1,0) txt.BackgroundTransparency = 1 txt.TextColor3 = Color3.fromRGB(255, 215, 0) txt.TextScaled = true txt.Font = Enum.Font.GothamBold txt.Text = "Chest" end
            end
        else
            for _, c in pairs(game:GetService("CollectionService"):GetTagged("_ChestTagged")) do if c:FindFirstChild("ESPC") then c.ESPC:Destroy() end end
        end
    end
end)

-- CHỐNG ADMIN & SERVER HOP 30 PHÚT
task.spawn(function()
...
game.StarterGui:SetCore("SendNotification", {Title = "Light Galaxy Hub V15", Text = "Đã Nạp Full 100% Sức Mạnh!", Duration = 5})
