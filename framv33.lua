-- ==============================================================================
-- LIGHT GALAXY HUB ◆ VERSION 30 PRO MAX (COMPLETE ULTIMATE EDITION)
-- 100% FULLY INTEGRATED - NO SHORTCUTS - MOBILE OPTIMIZED
-- DEVELOPMENT FOR PROFESSIONAL EXPLOITING & LEVEL FARMING SERVICES
-- ==============================================================================
...
_G.BringMob = true
_G.FastAtk = true
_G.AuraM1Fruit = false
_G.DragonStorm = false
_G.SmartPortal = true
...
_G.BoatSpeed300 = false
_G.BoatNoclip = false
_G.FindLeviMode = false
_G.AutoDriveTiki = false
_G.AutoBuyBoat = false
_G.SelectedBoat = ""
_G.TargetDangerLevel = "6"
_G.TargetSeaMob = ""

_G.AutoBuyChip = false
_G.StartRaidAfar = false
_G.AutoRaid = false
_G.AutoFruit = false
_G.AutoStoreFruit = false
_G.AutoRandomFruit = false
_G.KillAllBoss = false
_G.AutoRejoin30m = true

-- ==============================================================================
-- I. HỆ THỐNG GIAO DIỆN (UI CREATION - TRẮNG SÁNG & ĐEN KHÔNG ĐẤM MẮT)
-- ==============================================================================
local UI_Parent = game:GetService("CoreGui")
if not UI_Parent or not pcall(function() local a = UI_Parent.Name end) then 
...
local Hub = Instance.new("ScreenGui", UI_Parent)
Hub.Name = "LightGalaxyHub"
...
Title.Text = "LIGHT GALAXY HUB ◆ V30 PRO MAX (DEVELOPER EDITION)"
...
ToggleBtn.MouseButton1Click:Connect(function()
    ...
    ToggleBtn.Text = Main.Visible and "ẨN UI" or "MỞ UI"
end)

local TabSc = Instance.new("ScrollingFrame", Main)
TabSc.Size = UDim2.new(0, 160, 1, -50)
TabSc.Position = UDim2.new(0, 5, 0, 45)
TabSc.BackgroundTransparency = 1
TabSc.ScrollBarThickness = 2
Instance.new("UIListLayout", TabSc).Padding = UDim.new(0, 5)

local Cont = Instance.new("Frame", Main)
Cont.Size = UDim2.new(1, -175, 1, -50)
...
local function CreatePage(Name)
    local Pg = Instance.new("ScrollingFrame", Cont)
    Pg.Size = UDim2.new(1, 0, 1, 0)
    Pg.CanvasSize = UDim2.new(0, 0, 15, 0)
    Pg.BackgroundTransparency = 1
    Pg.ScrollBarThickness = 3
    Pg.Visible = false
    
    local Lyt = Instance.new("UIListLayout", Pg)
    ...
    local Btn = Instance.new("TextButton", TabSc)
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        Pg.Visible = true
        for _, b in pairs(TabSc:GetChildren()) do
...
local function AddToggle(Pg, Txt, Var)
    local Frm = Instance.new("Frame", Pg)
    Frm.Size = UDim2.new(1, -16, 0, 40)
    Frm.BackgroundColor3 = Color3.fromRGB(33, 33, 44)
    ...
    Btn.MouseButton1Click:Connect(function()
        _G[Var] = not _G[Var]
        Btn.BackgroundColor3 = _G[Var] and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(100, 100, 100)
        Btn.Text = _G[Var] and "ON" or "OFF"
    end)
end

local function AddLbl(Pg, Txt, VarStore)
    local Lbl = Instance.new("TextLabel", Pg)
    Lbl.Size = UDim2.new(1, -16, 0, 25)
    Lbl.BackgroundTransparency = 1
...
local function AddDropdown(Pg, Txt, List, Var)
  ...
  Lbl.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local MainBtn = Instance.new("TextButton", Frm)
    MainBtn.Size = UDim2.new(0.4, 0, 0, 24)
    MainBtn.Position = UDim2.new(0.6, -10, 0, 8)
    ...
    local DropScroll = Instance.new("ScrollingFrame", Frm)
    ...
    local function Populate()
        ...
        if #List > 0 then
            ...
            -- Danh sách Player động
            local rfs = Instance.new("TextButton", DropScroll)
            rfs.Size = UDim2.new(1, -10, 0, 25)
            rfs.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
            rfs.Text = "🔄 Làm Mới Danh Sách"
            rfs.TextColor3 = Color3.fromRGB(255, 255, 255)
            rfs.Font = Enum.Font.GothamBold
            rfs.TextSize = 10
...
-- ==============================================================================
-- II. THIẾT LẬP CÁC TRANG MENU CHUYÊN BIỆT
-- ==============================================================================
...
-- [TAB STATUS SERVER - CHỮ TRẮNG PHÂN SỐ 1/8 CHUẨN XỊN]
...
-- [TAB ESP]
AddToggle(pMisc, "ESP Player (Chữ Trắng - Mức Level & Khoảng Cách)", "ESPPlr")
AddToggle(pMisc, "Chống Admin & Auto Server Hop 30P", "AutoRejoin30m")

-- ==============================================================================
-- III. HỆ THỐNG BAY TỐI ƯU (SMART TWEEN & PORTAL C)
-- ==============================================================================
local function GetPortalFruit()
    local char = LocalPlayer.Character
    if char and char:FindFirstChild("Portal-Portal") then return char["Portal-Portal"] end
    if LocalPlayer.Backpack:FindFirstChild("Portal-Portal") then return LocalPlayer.Backpack["Portal-Portal"] end 
    return nil
end
...
local function SmartTween(TargetCFrame)
    ...
    -- KIỂM TRA SỬ DỤNG PORTAL BYPASS ĐỂ KHÔNG PHẢI TWEEN XA
    if _G.SmartPortal and Dist > 1800 then
        local PortalFruit = GetPortalFruit()
        if PortalFruit then
            Char.Humanoid:EquipTool(PortalFruit) 
            task.wait(0.2)
            pcall(function() 
                ReplicatedStorage.Modules.Net:FindFirstChild("RE/Shoot"):FireServer(TargetCFrame.Position, "C") 
            end) 
            task.wait(1.5) 
            return
        end
    ...
    -- CHẠY BODYVELOCITY KHÓA TRỌNG LỰC CHỐNG RƠI TUYỆT ĐỐI
...
-- ==============================================================================
-- IV. LÕI SÁT THƯƠNG (FAST ATTACK, DRAGON STORM VÀ M1 FRUIT AURA CHÈM NHANH)
-- ==============================================================================
...
task.spawn(function()
    while task.wait() do
        if _G.FastAtk or _G.DragonStorm or _G.AuraM1Fruit then
            pcall(function()
                local Char = LocalPlayer.Character
                -- Tự động cầm Trái Ác Quỷ nếu bật Aura M1 Fruit
                if _G.AuraM1Fruit then 
                ...
                -- Phóng to hitbox quái nếu bật Aura Dragon Storm diện rộng
                ...
                local c = CombatFramework.activeController 
                if c and c.blades[1] then
                    c.timeToNextAttack = 0 
                    c.hitboxMagnitude = _G.DragonStorm and 250 or 60
                end
            end)
        end
    end
...
-- ==============================================================================
-- V. LÕI HOOK SILENT AIM (BẺ CON TRỎ CHIÊU THỨC)
-- ==============================================================================
...
-- ==============================================================================
-- VI. HỆ THỐNG AUTO FARM QUEST LOGIC (SEA 3 FULLY EMBEDDED)
-- ==============================================================================
local function GetQuestLogic()
    ...
    if Lvl >= 1500 and Lvl <= 1525 then 
        QuestName = "Pirate Millionaire Quest" QuestNum = 1 MobName = "Pirate Millionaire" QuestCFrame = CFrame.new(-318, 44, 5972) MobCFrame = CFrame.new(-318, 44, 5972)
    elseif Lvl >= 1525 and Lvl <= 1575 then 
        QuestName = "Pistol Billionaire Quest" QuestNum = 1 MobName = "Pistol Billionaire" QuestCFrame = CFrame.new(-462, 73, 5325) MobCFrame = CFrame.new(-462, 73, 5325)
    elseif Lvl >= 1575 and Lvl <= 1625 then
        QuestName = "DragonCrewQuest" QuestNum = 1 MobName = "Dragon Crew Warrior" QuestCFrame = CFrame.new(6338, 52, -1213) MobCFrame = CFrame.new(6338, 52, -1213)
    else 
    ...
    if qName ~= "Bypass" then
        if not LocalPlayer.PlayerGui.Main.Quest.Visible then
            ...
            for _, E in pairs(Workspace.Enemies:GetChildren()) do 
                if string.find(E.Name, mName) and E.Humanoid.Health > 0 then tMob = E break end 
            end
            if tMob then 
...
-- ==============================================================================
-- VII. VÒNG LẶP CHẠY LÕI TỰ ĐỘNG CHÉM, GOM QUÁI (BRING MOB) VÀ PHỤ BẢN
-- ==============================================================================
            ...
            for _, Enemy in pairs(Workspace.Enemies:GetChildren()) do
                if Enemy:FindFirstChild("HumanoidRootPart") and Enemy:FindFirstChild("Humanoid") and Enemy.Humanoid.Health > 0 then
                    if (Enemy.HumanoidRootPart.Position - MyRoot.Position).Magnitude < 350 then
                        ...
                        -- Gom quái ra trước mặt 15m để chém mượt, không để dưới chân tránh kẹt map
...
-- Thực thi tác vụ cày cuốc liên tục
task.spawn(function()
    while task.wait(0.2) do
        ...
        if _G.FarmBone then 
            pcall(function() 
                local t = nil 
                for _, E in pairs(Workspace.Enemies:GetChildren()) do 
                    if (E.Name == "Reborn Skeleton" or E.Name == "Living Zombie") and E.Humanoid.Health > 0 then t = E break end 
                end 
                if t then SmartTween(t.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) else SmartTween(CFrame.new(-9481, 142, 5566)) end 
            end) 
        end
        if _G.FarmCake then 
            pcall(function() 
                local t = nil 
                for _, E in pairs(Workspace.Enemies:GetChildren()) do 
                    if string.find(E.Name, "Cake") and E.Humanoid.Health > 0 then t = E break end 
                end 
                if t then SmartTween(t.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) else SmartTween(CFrame.new(-2022, 38, -12028)) end 
            end) 
        end
        if _G.KillAllBoss then
            pcall(function()
                for _, mob in pairs(Workspace.Enemies:GetChildren()) do
                    if mob:FindFirstChild("HumanoidRootPart") and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:GetAttribute("IsBoss") then
                        SmartTween(mob.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) break
                    end
                end
            end)
        end
...
-- ==============================================================================
-- VIII. HỆ THỐNG LEVIATHAN HUNT & SEA EVENT THÔNG MINH (BẢN SĂN LƯỚT 100%)
-- ==============================================================================
...
local function SmartSeaEventDetect()
    ...
    for _, e in pairs(Workspace.Enemies:GetChildren()) do
        if e:FindFirstChild("HumanoidRootPart") and e:FindFirstChild("Humanoid") and e.Humanoid.Health > 0 then
            ...
            if string.find(name, "terror") or string.find(name, "shark") or string.find(name, "ghost") or string.find(name, "piranha") then 
                detected = true threatName = e.Name break 
            end
        end
    end
    if not detected and Workspace:FindFirstChild("SeaBeasts") then
        for _, sb in pairs(Workspace.SeaBeasts:GetChildren()) do 
            if sb:FindFirstChild("HumanoidRootPart") and sb.Humanoid.Health > 0 then detected = true threatName = "Sea Beast" break end 
        end
    end
        ...
        if Seat and Seat.Occupant and Seat.Occupant.Parent == LocalPlayer.Character then
            ...
            if _G.FindLeviMode then
                ...
                if hasThreat and threatName ~= "Leviathan" then
                    -- BỐC ĐẦU BAY THUYỀN LÊN 500M ĐỂ NÉ TERRORSHARK/SHARK
                    ...
                    -- Không có mối đe dọa thì hạ thuyền xuống mặt nước đi tiếp
                    if MyBoat.PrimaryPart.Position.Y > 50 then 
                        MyBoat.PrimaryPart.CFrame = CFrame.new(MyBoat.PrimaryPart.Position.X, 20, MyBoat.PrimaryPart.Position.Z) * CFrame.Angles(0, math.rad(MyBoat.PrimaryPart.Orientation.Y), 0) 
                    end
                end
            end
        end
            ...
            pcall(function()
                ...
                if not MyBoat then
                    ...
                    if Char and Char:FindFirstChild("HumanoidRootPart") then
                        if (Char.HumanoidRootPart.Position - TikiOutpostCFrame.Position).Magnitude > 50 then 
                            SmartTween(TikiOutpostCFrame * CFrame.new(0, 10, 0)) 
                        else 
                            if _G.SelectedBoat ~= "" then 
                                CommF:InvokeServer("BuyBoat", _G.SelectedBoat) task.wait(1) 
                            end 
                        end
                    end
                else
                    local Seat = MyBoat:FindFirstChild("VehicleSeat") 
                    local Char = LocalPlayer.Character 
                    if Seat and not Seat.Occupant and Char and Char:FindFirstChild("HumanoidRootPart") then 
                ...
                if target then
                    ...
                    if _G.SpamMelee then 
                        for _, k in ipairs({Enum.KeyCode.Z, Enum.KeyCode.X, Enum.KeyCode.C, Enum.KeyCode.V}) do 
                            VIM:SendKeyEvent(true, k, false, game) task.wait(0.1) VIM:SendKeyEvent(false, k, false, game) 
                        end 
                    end
                else
                    local MyBoat = GetMyBoat()
...
-- ==============================================================================
-- IX. PHỤ TRỢ (TRADE BONE, ĐỒNG BỘ STATUS & ESP PLAYER CHỮ TRẮNG)
-- ==============================================================================
task.spawn(function()
    while task.wait(1) do
        ...
        if _G.AutoFruit then
            pcall(function()
                for _, f in pairs(Workspace:GetChildren()) do
                    if f:IsA("Tool") and string.find(f.Name, "Fruit") and f:FindFirstChild("Handle") then
                        SmartTween(f.Handle.CFrame) task.wait(0.5)
                        if _G.AutoStoreFruit then CommF:InvokeServer("StoreFruit", f:GetAttribute("OriginalName") or f.Name, f) end
                    end
                end
            end)
        end
...
task.spawn(function()
    while task.wait(1.5) do
        pcall(function()
            ...
            if Lighting:FindFirstChild("Sky") then
                ...
                if currentPhase == 8 or string.find(moonTex, "9709149431") or string.find(moonTex, "9709135895") then 
                    _G.lblMoonTime.Text = "Trăng: 8/8 (Full Moon up v4 - Ánh sáng xuyên qua mây)" 
                else 
                    _G.lblMoonTime.Text = "Trăng: " .. currentPhase .. "/8" 
                end
            else 
                _G.lblMoonTime.Text = "Trăng: Lỗi load bầu trời" 
            end
            ...
            -- CÁC ĐẢO DANGER 6 & SỰ KIỆN LEVIATHAN
        ...
        -- ESP CHỮ TRẮNG PHÁT SÁNG ĐỈNH CAO CHUYÊN NGHIỆP
        if _G.ESPPlr then
            pcall(function()
                for _, p in pairs(Players:GetPlayers()) do
