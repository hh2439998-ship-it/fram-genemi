-- ==============================================================================
-- LIGHT GALAXY HUB - PERFECT KILL AURA V5 (FIX 100% LỖI ĐỨNG IM TRÊN ĐIỆN THOẠI)
-- PURE LUAU UI - 5 LAYER AUTO CLICK - REAL BRING MOB - MOBILE OPTIMIZED
-- ==============================================================================
...
Title.Text = "LIGHT GALAXY HUB ◆ PERFECT KILL AURA V5"
...
-- ==============================================================================
-- IV. ĐỘC QUYỀN: HỆ THỐNG FAST ATTACK (5 LỚP) & REAL BRING (HOÀN HẢO 100%)
-- ==============================================================================

-- 1. Xóa Hoạt Ảnh chuẩn xác (Lấy theo Name thay vì ID)
    ...
    if Hum then
        Hum.AnimationPlayed:Connect(function(Track)
            if _G.FastAttack then
                ...
                if string.find(animName, "attack") or string.find(animName, "slash") or string.find(animName, "punch") or string.find(animName, "m1") or string.find(animName, "combat") then
                    Track:Stop() 
                end
            end
        end)
...
-- 2. Core Fast Attack (Đã tách rời 5 khối pcall để chống tắc nghẽn lệnh)
spawn(function()
    local VirtualUser = game:GetService("VirtualUser")
    while task.wait() do
        if _G.FastAttack then
            
            -- LỚP 1: Tự động móc vũ khí ra cầm
            pcall(function()
                local Char = LocalPlayer.Character
            ...
            -- LỚP 2: Chém bằng CombatFramework (Đâm dame gốc siêu to)
            pcall(function()
            ...
            -- LỚP 3: Chém bằng API Mặc định của Roblox (Cứu cánh cực mạnh)
            pcall(function()
                local Char = LocalPlayer.Character
                if Char then
            ...
            -- LỚP 4: Giả Lập Click Ẩn (VirtualUser)
            pcall(function()
                VirtualUser:CaptureController()
                VirtualUser:ClickButton1(Vector2.new(850, 850))
            end)
            
            -- LỚP 5: Chạm Màn Hình Trực Tiếp (VIM)
            pcall(function()
